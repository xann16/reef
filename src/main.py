...
@click.pass_context
@click.option('--config-path', '-p', help="Path to directory containing reef config (default: ~/.reef)",
    type=click.Path(exists=False), default=path.abspath(path.expanduser('~/.reef')))
@click.option('--exec-path', '-x', help="Path to directory with reef executable shell script (default: current directory)",
